    ...
    <main className="min-h-screen bg-slate-950 px-4 py-8 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-800 pb-5">
            <div className="flex items-center gap-3">
              <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-full bg-blue-600 text-3xl font-bold text-white ring-2 ring-blue-400/40">
        ...
        <header className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
          ...
          <div className="flex items-center gap-3">
            <button type="button" aria-label="Open profile navigation" onClick={() => setNavigationOpen(true)} className="flex h-20 w-20 items-center justify-center rounded-full bg-blue-600 text-3xl font-bold text-white ring-2 ring-blue-400/40 transition hover:bg-blue-500">
        ...
        <section className="mt-8 rounded-2xl border border-yellow-500/20 bg-yellow-500/5 p-6"><p className="text-sm font-semibold uppercase tracking-wider text-yellow-400">Recommended Next Step</p><h2 className="mt-2 text-2xl font-bold">{data.nextAction.title}</h2><p className="mt-2 text-slate-400">{data.nextAction.description}</p><Link href={data.nextAction.href === "/ai-result" && data.skills.length < 3 ? "/assessment" : data.nextAction.href} className="mt-5 inline-block rounded-xl bg-yellow-500 px-6 py-3 font-bold text-slate-950">Continue →</Link></section>
